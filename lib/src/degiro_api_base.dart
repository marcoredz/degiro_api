...
class DegiroApi {
  ...
  Future<String> setOrder({
    ...
    required int size,
    int timeType = TimeTypes.day,
    double? price,
    double? stopPrice,
  }) async {
    // Check order request
    final requestBody = CheckOrderRequestBody(
      buySell: buySell,
      orderType: orderType,
      productId: productId,
      timeType: timeType,
    ...
    final String validate = _validateCheckOrderRequest(requestBody);
    if (validate.isNotEmpty) {
      throw DegiroApiError(message: validate)..methodName = "checkOrder";
    }

    final result = await _repository.checkOrderRequest(
      sessionId,
      accountInfo.intAccount,
      requestBody,
    );
    ...
    return result.when(
      (error) => throw error..methodName = "checkOrder",
      (confirmationId) {
        // continua ...
        // confirm order
      },
  ...
  String _validateCheckOrderRequest(CheckOrderRequestBody body) {
    if (body.orderType != OrderTypes.limited &&
        body.orderType != OrderTypes.market &&
        body.orderType != OrderTypes.stopLoss &&
        body.orderType != OrderTypes.stopLossLimit) {
      return 'Invalid order type';
    }

    if (body.timeType != TimeTypes.day && body.timeType != TimeTypes.continuos) {
